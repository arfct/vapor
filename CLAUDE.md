...
Work is tracked in GitHub issues on this repository; branches are `feat/<issue>-<slug>`. Issues and PRs go to `origin` (`arfct/vapor`), never to the `upstream` remote (`inanimate-tech/mist`): always pass `--repo arfct/vapor` to `gh issue create` / `gh pr create`, and check `gh repo set-default --view` before filing anything, since `gh` can default to upstream in a fork. Maintainers at Artifact additionally follow the [Artifact Primer](https://github.com/arfct/ops/tree/main/primer) for style, commits, and deployment; contributors to a fork don't need it.
...
| `/oauth/*`, `/.well-known/oauth-*`, `/.well-known/openid-configuration` | `workers/oauth.ts` — OAuth 2.1 AS for MCP, plus OIDC discovery and `/oauth/userinfo` (sub/email/email_verified); `scope` echoes honoured OpenID scopes + granted caps |
| `/.well-known/openai-apps-challenge` | `workers/routes.ts` — serves `OPENAI_APPS_CHALLENGE` for ChatGPT plugin-directory domain verification (#103) |
...
- **Mentions** (#116) — where an `@agent` is written decides what it does. In a comment or a thread reply it records a `mention` event every time (the threads observer, on a new thread's `commentText` or a new reply). In the body it is a pointer, not a notification: `inviteMentioned` fires one `mention` (`invite: true`) per document, only while the agent is not present (joined and not idle) and only if `roster.invited_at` is null, then stamps it; policy in `app/shared/mention-policy.ts`. Inline `{>>…<<}` runs are stripped before the body scan so a comment is not counted twice. `read_document` returns `mentions`, the blocks that name the caller, so a present agent finds what it should look at.
