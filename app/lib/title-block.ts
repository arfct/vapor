...
/** True while nothing has been written after the first block. */
function restIsEmpty(doc: PMNode): boolean {
  for (let i = 1; i < doc.childCount; i++) if (doc.child(i).textContent.length > 0) return false;
  return true;
}

/**
 * Placeholder for an empty top-level block: the title text on the first
 * line while the document is still (nearly) empty, the body text on the
 * line after a title while nothing else has been written. Empty string
 * otherwise. StarterKit's TrailingNode keeps an empty paragraph after the
 * last block, so "nothing else" means every block after the first is empty.
 */
export function placeholderFor(doc: PMNode, index: number, text: TitleBlockOptions): string {
  ...
  if (!first || doc.child(1).type.name !== "paragraph") return "";
  if (first.type.name !== "heading" && first.textContent.length > 0) return "";
  return restIsEmpty(doc) ? text.body : "";
}

/**
 * Both placeholders are visible from the start. The empty first block is
 * styled as a title (`is-title`) so the hint sits at heading size; when
 * the document has only that one block, the body hint is a widget after
 * it — there is no second block to decorate yet.
 */
function placeholderDecorations(doc: PMNode, text: TitleBlockOptions): DecorationSet {
  ...
  doc.forEach((node, offset, index) => {
    ...
    const cls = index === 0 ? "is-empty is-title" : "is-empty";
    decorations.push(
      Decoration.node(offset, offset + node.nodeSize, { class: cls, "data-placeholder": placeholder }),
    );
  });
  const first = doc.firstChild;
  if (doc.childCount === 1 && first && first.childCount === 0 && !first.isLeaf) {
    decorations.push(
      Decoration.widget(
        first.nodeSize,
        () => {
          const el = document.createElement("p");
          el.className = "placeholder-body";
          el.setAttribute("contenteditable", "false");
          el.textContent = text.body;
          return el;
        },
        { side: 1, ignoreSelection: true, key: "placeholder-body" },
      ),
    );
  }
 ...
 * text. Title / body placeholders are decorations computed from the
 * rendered state — never content, never selectable.
 */
