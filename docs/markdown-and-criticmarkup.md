...
An image may carry a Pandoc-style attribute block giving its width and its alignment, `![cat.png](/abcd1234/attachments/<id>/cat.png){width=50% align=left}`. Both keys are optional. With no `width` the image renders at its natural size, capped by the column; `width` is a percent from 1% to 100% or the token `full`, which fills the page past the text column. `align` is `left`, `center`, or `right`, and left and right float the image so body text wraps beside it until the next heading, rule, table, code block, or blockquote. A floated image is capped at a third of the column whatever its `width` says, so the text beside it keeps a readable measure, and every image is capped at 80% of the viewport height, so a tall one cannot take the screen. Both caps hold the aspect ratio: a 400x3000 image at `width=100%` renders 82px wide, because the height is the binding constraint. A full-width image ignores `align`, having no room to wrap beside it, and below 640px alignment drops and every image takes its own line. Unknown keys and out-of-range values are dropped on parse rather than carried. In the EPUB and on the print page `full` means 100% of the text width, not wider: those pages are a centred 42em column with no canvas outside it.

The image bubble sets no percentages: it toggles full bleed and picks an alignment, and a width other than `full` comes from the markdown. The filename is the image's `alt`; it is not drawn under the image.
