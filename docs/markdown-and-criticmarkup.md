...
An image may carry a Pandoc-style attribute block giving its width and its alignment, `![cat.png](/abcd1234/attachments/<id>/cat.png){width=50% align=left}`. `width` is a percent from 1% to 100% or the token `full`, which fills the page past the text column; `align` is `left`, `center`, or `right`, and left and right float the image so body text wraps beside it until the next heading, rule, table, code block, or blockquote. A full-width image ignores `align`, having no room to wrap beside it, and below 640px alignment drops and every image takes its own line. Unknown keys and out-of-range values are dropped on parse rather than carried. In the EPUB and on the print page `full` means 100% of the text width, not wider: those pages are a centred 42em column with no canvas outside it.
