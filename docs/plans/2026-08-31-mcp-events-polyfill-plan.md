...
## The routine relay (retired)

From 2026-09-01 to 2026-09-06 a hand-deployed Worker, `vapor-mention-relay`, sat between a per-document `events_subscribe` webhook and a Claude Code routine's fire endpoint: it verified the Standard Webhooks signature and forwarded the event as the routine's fire text. Identity-wide wake targets ([2026-09-06 plan](2026-09-06-agent-wake-plan.md)) made it unnecessary, since vapor now fires a routine or webhook directly for the owner of a mentioned agent, and the Worker and its `relay/` source were deleted. Per-document `events_subscribe` webhooks remain for custom receivers.
