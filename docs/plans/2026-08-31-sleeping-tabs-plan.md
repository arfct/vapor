...
- A weekly (or on-demand) GraphQL query of DO duration/requests/rows against budget, recorded in the repo (script in `tools/` or a doc), so the next quota cliff is visible days out, not at 500-time.
