...
- **Every image is capped at 80% of the viewport height**, in `app.css` and in `READING_CSS`, so the EPUB and print pages carry it too. The cap holds the aspect ratio rather than letterboxing, so it can override a stated width: a 400x3000 image at `width=100%` renders 82px wide in a 630px column.
