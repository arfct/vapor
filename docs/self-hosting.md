...
**Send to Kindle** needs the instance to send email. Two more optional values enable it: `SEND_FROM_EMAIL`, the address documents are sent from (a var), and `RESEND_API_KEY`, an API key for [Resend](https://resend.com) whose account has that address's domain verified (a secret: `wrangler secret put RESEND_API_KEY`). With either unset the Kindle row in Share → Send to device offers the EPUB download and Amazon's upload page instead. Readers add your sender address to their Amazon approved senders once; the dialog shows them the exact address. Each message is From `"<Reader's name> via vapor" <your address>` with Reply-To set to the reader's own email, so the approved address never changes while replies still reach the person who sent it. Send to reMarkable needs nothing from the operator.
